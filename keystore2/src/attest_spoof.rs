...
fn real_boot_hash() -> Vec<u8> {
    ...
    rustutils::android::system_properties::read("ro.boot.vbmeta.digest")
        .ok()
        .flatten()
...
fn read_patch_levels() -> (i64, i64) {
    let sp = rustutils::android::system_properties::read("ro.vendor.build.security_patch")
        .ok()
        .flatten()
        .filter(|s| !s.is_empty())
        .or_else(|| rustutils::android::system_properties::read("ro.build.version.security_patch").ok().flatten())
        .unwrap_or_default();
